...
A \index{countable graph}\emph{countable graph} is a graph with a countable set of vertices;
...
The property in the definition will be called the \index{Rado property}\emph{Rado property}; so we can say that \textit{for the sets of vertices $V$ and $W$ in a graph, the Rado property holds} or \textit{does not hold}.
...
\begin{thm}{Exercise}\label{ex:rado-partition}
...
By construction, the Rado property holds in $G'$ for any two sets $V$ and $W$ of vertices in $G$ --- the required vertex $v$ is the vertex in $G'$ that corresponds to the subset $V$.

Let $G_1$ be a graph with one vertex.
\begin{figure}[ht!]%{r}{20 mm}
\vskip-0mm
\centering
...
\begin{thm}{Exercise}\label{ex:rado-costructive}
...
\parit{Proof.}
It is sufficient to show that for two given finite sets of vertices $V$ and $W$, the Rado property fails with probability 0.
...
The {}\emph{Rado graph} is also called the \index{Erd\H{o}s–R\'enyi graph}\emph{Erd\H{o}s–R\'enyi graph} or \index{random graph}\emph{random graph};
