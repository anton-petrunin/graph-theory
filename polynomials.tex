...
\[M_{K_{p+1}}(x)=M_{K_{p}}(x)+n\cdot x\cdot M_{K_{p-1}}(x).\]
...
Therefore it suffices to prove that all monomials in $S_{K_p}$ without $x_p$ sum up to $(x_1+\dots+x_{p-1})^{p-2}$.
...
by adding an edge from $x_n$ to $x_i$ with $i<p$.
