...
If an alternated path path connects two unmatched vertexes of $G$, then it is called \index{augmenting path}\emph{$M$-augmenting}.
...
Let us construct the set $S$ by taking one vertex (left or right) of each edge $m$ in $M$ by the following rule:
...
$P_1$ is marked by solid line and $Q$ is marked by dashed line.)
