...
In fact the travelling salesman problem is a classical example of the so called {}\emph{NP-hard problems}; it means that if you find a fast algorithm to solve it, then you solve the \index{P=NP problem}\emph{P=NP problem}, which is the most important question in modern mathematics.

The so called \index{nearest neighbour algorithm}\emph{nearest neighbour algorithm} gives a heuristic way to find a Hamilton cycle with small (but not necessary smallest weight).
...
The nearest neighbour algorithm is an example of \index{greedy algorithm}\emph{greedy algorithms}; in other words, it chooses a cheapest step at each stage.
