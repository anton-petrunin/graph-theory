...
The so called \index{travelling salesman problem}\emph{travelling salesman problem} gives a classical example of optimization problem.
...
In fact the travelling salesman problem is a classical example of the so called {}\emph{NP-hard problems}; it means that if you find a fast algorithm to solve it, then you solve the \index{P=NP problem}\emph{P=NP problem} --- the most important question in modern mathematics.
...
\item The \index{assignment problem}\emph{assignment problem}  asks for a matching of a given size with minimal weigh, in a bipartite graph.
