...
and finite collection of \index{edge}\emph{edges}, each edge connects two vertexes (in the example above, edge is a regular flight).
A pair of vertexes can be connected by few edges, such edges are called \index{parallel edges}\emph{parallel} (in our example, it might mean that the airline makes few  flight a day between these airports). 
Also an edge can connect a vertex to itself, such edge is called a \index{loop}\emph{loop} (we might think of it as а sightseeing flight).
...
A vertex with zero degree is called \index{isolated vertex}\emph{isolated} and a vertex of degree one is called \index{end vertex}\emph{end vertex}.

\smallskip

A pseudograph without loops is also called \index{multigraph}\emph{multigraph}.
A multigraph without parallel edges is also called \index{graph}\emph{graph}.
Most of the time we will work with graphs.

If $x$ and $y$ are vertexes of a pseudograph $G$, we say $x$ is \index{adjacent}\emph{adjacent} to $y$ if there is an edge between $x$ and $y$.
